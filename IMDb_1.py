...
token = Tokenizer(num_words=2000) #利用 Tokenizer 建立一個 token，參數為建立 2000 個字的字典
token.fit_on_texts(train_text) #讀取訓練資料的文字內容，並依照出現次數將前 2000 個單字列進字典裡
...
x_train = sequence.pad_sequences(train_text_seq,maxlen=100)
x_test = sequence.pad_sequences(test_text_seq,maxlen=100)
#maxlen : 設定轉換後的文字長度，maxlen = 100 讓轉換後的文字長度為 100
#文字長度不足 100 的部分，在前面用 0 補齊；文字長度超過 100 的資料，則從前面開始截去，留下後面的 100
...
model.add(Embedding(output_dim=32, #輸出的維度，將「數字List」轉換為 32 維度的向量
                    input_dim=2000, #輸入的維度，先前建立存放 2000 個字的字典
                    input_length=100)) #「數字List」的長度，每一筆資料的長度為 100

#加入「Drop層」，其每次訓練迭帶時隨機捨棄隱藏層 20% 的神經元，避免「過擬合 overfitting」
model.add(Dropout(0.2))

#加入「平坦層」
#將「數字List」每一筆 100 個數字的 32 維度向量轉換為 3200 ( 100 * 32 ) 個神經元
model.add(Flatten())
...
#儲存訓練模型，參數為欲存放的位置
model.save('IMDb/IMDb_1_model.h5')
#讀取訓練模型，參數為模型的位置'''
model = load_model('IMDb/IMDb_1_model.h5')
