...
#「RNN 遞歸神經網路」，使模型具有「記憶」，讓參數具有順序姓
#「MLP 多層感知器」與「CNN 卷積神經網路」都只能依照當下狀態進行辨識
#RNN 模型的原理
#St (隱藏的狀態，代表神經網路的「記憶」) = ( [U]Xt + [W]St-1 ) (t為時間點) 
#但是「RNN」在訓練時會有「long-term dependencies」的問題，當間隔不斷擴大時，模型梯度會持續遞增或遞減，導致無窮大或趨近零，喪失學習能力
...
#加入「RNN層」
model.add(SimpleRNN(units=16)) #定義「RNN層」擁有 16 個記憶神經元
...
#儲存訓練模型，參數為欲存放的位置
model.save('IMDb/IMDb_RNN_model.h5')
#讀取訓練模型，參數為模型的位置'''
model = load_model('IMDb/IMDb_RNN_model.h5')
